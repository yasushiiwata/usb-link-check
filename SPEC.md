...
`Usb300` から求めた P は「5 Gbps 以上・上限不明」である（4.2）。判定表の比較は次の規則で行う。

| 条件 | 判定 |
|---|---|
| `L < P の下限値` | 表のとおり（`L < min(P, D)` なら A1） |
| `L = P の下限値` かつ `L = D` | **A4**。`min(P, D) = D = L` なので、P の上限が不明でも最高速を達成している |
| `L = P の下限値` かつ `L < D` かつ P が `AT_LEAST` | **A5（下記）。A1 と A2 を区別できない** |
| `L = P の下限値` かつ `L < D` かつ P が `EXACT` | A2 |

| # | 条件 | 結論 | 提案 | 確度 |
|---|---|---|---|---|
| A5 | `L = P の下限値 < D` かつ P が `AT_LEAST` | **ポートまたはケーブルが律速（切り分け不能）** | 「既知の高速ケーブルに差し替えて再測定してください。速度が上がればケーブル、変わらなければポート側が原因です」 | `unknown` |

A5 は `UNDETERMINED`（終了コード 3）とする。ポートが実際に何 Gbps まで出せるか（5 / 10 / 20）を Windows の API から知る手段が無いため、断定せず B1 と同じ「実験で切り分ける手順」を渡す。
