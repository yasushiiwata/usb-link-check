...
| マーク | 条件 | 意味 |
|---|---|---|
| `⚠` | `L < min(P, D)` の下限値 | 改善の余地があることが確定している |
| `?` | 上記に該当せず、P または D が `AT_LEAST` / `UNKNOWN` | 判別できない（能力の上限が不明なため、落ちているのか天井なのか分からない） |
| （無印） | `min(P, D)` が `EXACT` で `L = min(P, D)` | 現構成で出せる最高速で動作していることが確定している |

  - 無印の意味は「**現構成では**最高速」である。ポートが律速している構成（判定表 A2）も `L = min(P, D)` なので無印になる。凡例でその旨を示し、「問題なし」と読めないようにする。
  - `min` の確度は 5.1 の一般則で求める。個々の P / D の確度ではなく `min` の確度で判断する（例: P ≥ 5 Gbps かつ D = 5 Gbps（EXACT）なら `min` は確定値なので、`L = 5 Gbps` は無印）。
