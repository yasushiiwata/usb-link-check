...
`--list` は各デバイスの L（実効リンク速度）、P（ポート能力）、D（デバイス能力）を並べ、各行の状態を 3 値で示します。判断できない行を「問題なし」と読ませないためです。判定基準は D（デバイス能力）で、`⚠` の付く行は `--device` でも「改善の余地がある」と判定されます。

```
    VID:PID    L(実効)     P(ポート)   D(デバイス)  位置                デバイス名
    32E6:9221  480 Mbps    5 Gbps+     480 Mbps    ハブ0/ポート6       Web Camera
⚠   346D:5678  480 Mbps    480 Mbps    5 Gbps      ハブ0/ポート8       Acer USB Flash Drive
?   04F2:0400  1.5 Mbps    5 Gbps+     1.5 Mbps+   ハブ0/ポート12      Chicony USB Keyboard

⚠ = デバイスはもっと速く動けます（L < D が確定）。改善の余地があります
? = 判別できません（D の上限が不明で、デバイスが天井なのか落ちているのか分からない）
無印 = デバイス自身の上限で動作しています（L = D が確定。これ以上速くなりません）
```

上の例の USB メモリは、5 Gbps 対応なのに USB2 専用のコネクタに挿さっているため 480 Mbps で動いています。挿す穴を変えれば速くなるので `⚠` が付きます。
